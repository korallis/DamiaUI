...
local FONT_FACE = "Interface\\AddOns\\ColdMisc\\media\\homespun.ttf" --what font to use
local FONT_SIZE = 10 --the base font size to use at a scale of 1
local MIN_SCALE = 0.5 --the minimum scale we want to show cooldown counts at, anything below this will be hidden
local MIN_DURATION = 2.5 --the minimum duration to show cooldown text for
local EXPIRING_DURATION = 1.5 --the minimum number of seconds a cooldown must be to use to display in the expiring format
...
--  new function registers due to 4.3 updates (cds were having strange issues, so i have to "manually" update them)
local active = {}
local hooked = {}
